...
function App() {
  ...
  const downloadReport = () => {
    ...
    doc.text('DeepScan AI Forensics Laboratory', 105, 20, { align: 'center' });
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    doc.text('Official Digital Evidence Report', 105, 30, { align: 'center' });
    
    // Evidence Details
    doc.setTextColor(15, 23, 42); // Slate 900
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('1. Digital Evidence Metadata', 14, 55);
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['SHA-256 Hash', result.file_hash ? result.file_hash : 'N/A (Streaming Mode)'],
      ],
      theme: 'grid',
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('2. Neural Inference Verdict', 14, finalY1);
    
    const verdictColor = isDeepfake ? [239, 68, 68] : [16, 185, 129];
    autoTable(doc, {
      startY: finalY1 + 5,
      head: [['Classification', 'AI Confidence', 'Conclusion']],
      body: [
        [
          result.verdict, 
          `${(result.confidence * 100).toFixed(2)}%`,
          isDeepfake ? 'High likelihood of synthetic AI generation.' : 'High likelihood of authentic human speech.'
        ]
      ],
      theme: 'grid',
      headStyles: { fillColor: verdictColor as [number, number, number] }
    });
    ...
    doc.text('3. Forensic Sub-Metrics', 14, finalY2);
    
    const acousticConsistency = isDeepfake ? (100 - (result.confidence * 100) + 20).toFixed(2) : (result.confidence * 100).toFixed(2);
    const tempVar = result.temporal_variance ? result.temporal_variance.toFixed(4) : 'N/A';
    const tempArtifacts = isDeepfake ? (result.confidence > 0.85 ? 'HIGH' : 'MEDIUM') : 'LOW';
    
    autoTable(doc, {
      startY: finalY2 + 5,
      head: [['Metric Layer', 'Score / Value', 'Description']],
      body: [
        ['Acoustic Consistency', `${acousticConsistency}%`, 'Phonetic anomalies and spectral glitches.'],
        ['Temporal Variance', tempVar, 'Variance of sequence embeddings across time steps.'],
        ['Temporal Artifacts', tempArtifacts, 'Unnatural prosody or robotic cadences.'],
        ['FAISS Database Match', result.faiss_memory?.match_verdict || 'None', 'Nearest match in known forensic signatures database.'],
        ['FAISS Similarity', `${result.faiss_memory?.similarity.toFixed(2) || '0'}%`, 'Acoustic distance from the nearest signature.'],
      ],
      theme: 'striped',
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text('4. Security Recommendations', 14, finalY3);
    
    doc.setFontSize(12);
    doc.setFont('helvetica', 'normal');
    if (isDeepfake) {
      doc.setTextColor(220, 38, 38);
      doc.text('WARNING: Do not trust the speaker in this audio. Do not send money or sensitive', 14, finalY3 + 10);
      doc.text('information. We recommend blocking the contact and treating this as a social', 14, finalY3 + 16);
      doc.text('engineering attack.', 14, finalY3 + 22);
    } else {
      doc.setTextColor(5, 150, 105);
      doc.text('This audio shows no signs of digital manipulation or synthetic generation.', 14, finalY3 + 10);
      doc.text('It is highly likely to be authentic.', 14, finalY3 + 16);
    }
    
    // Save
    doc.save(`DeepScan_Report_${result.filename.replace(/\.[^/.]+$/, "")}.pdf`);
  };
    ...
    <div className="page-wrapper">
      ...
      <nav className="navbar">
        ...
        <div className="nav-links">
          <a href="/">Dashboard</a>
          <a href={`${API_URL}/docs`} target="_blank" rel="noopener noreferrer">API Documentation</a>
